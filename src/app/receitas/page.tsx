...
export default function ReceitasPage() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" style={{ backgroundColor: '#0F0F15' }}>
      <header className="flex items-center justify-between px-4 md:px-6 py-4 border-b flex-shrink-0 gap-3" style={{ borderColor: '#1E1E28' }}>
        <div className="min-w-0">
          <h1 className="text-base font-semibold" style={{ color: '#F0F0F5' }}>Receitas</h1>
          <p className="text-xs hidden md:block" style={{ color: '#6B6B80' }}>Acompanhe todas as suas entradas de dinheiro.</p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <MonthPicker align="right" />
          <button onClick={() => setShowModal(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold" style={{ backgroundColor: '#C9A86A', color: '#1B1B1F', whiteSpace: 'nowrap' }}>
            <Plus size={13} />
      ...
      <main className="flex-1 overflow-auto p-4 md:p-6 flex flex-col gap-4 md:gap-5">
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4" style={{ gridTemplateColumns: undefined }}>
